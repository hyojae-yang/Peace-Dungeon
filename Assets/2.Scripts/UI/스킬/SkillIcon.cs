...
public class SkillIcon : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler
{
    ...
    [Tooltip("플레이어의 스탯을 관리하는 PlayerStats 스크립트")]
    ...
    public void OnPointerEnter(PointerEventData eventData)
    {
        ...
        {
            // playerStats에서 현재 스킬 레벨을 가져옵니다.
            int currentLevel = 0;
            if (playerStats.skillLevels.ContainsKey(skillData.skillId))
            {
                currentLevel = playerStats.skillLevels[skillData.skillId];
            }
