...
public class EquipmentSlotUI : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler
{
    ...
    // private GameObject instantiatedTooltip; // (기존) 이 변수는 아래 static 변수로 대체되었습니다.
    private PlayerCharacter playerCharacter;

    /// <summary>
    /// 현재 활성화된 장비 툴팁 인스턴스입니다.
    /// 장비 창 툴팁은 씬에 단 하나만 활성화되므로 static으로 관리하며,
    /// 장비 UI를 닫는 상위 컨트롤러(예: EquipmentUIController)에서 접근하여 강제 파괴할 수 있도록 합니다. (문제 해결의 핵심)
    /// </summary>
    ...
    {
        // 장착된 아이템이 있고, 툴팁 프리팹이 할당되어 있다면
        if (currentEquippedItem != null && tooltipPrefab != null)
        {
            // [수정된 로직 1] 새로운 툴팁을 생성하기 전에, 현재 활성화된 장비 툴팁이 있다면 파괴하여 잔상 및 중복을 방지합니다.
            if (currentActiveEquipTooltip != null)
            {
                Destroy(currentActiveEquipTooltip);
                currentActiveEquipTooltip = null; // 정적 참조 해제
            }
    ...
    {
        // [수정된 로직 2] static 변수를 사용하여 툴팁을 파괴하고 참조를 해제합니다.
        if (currentActiveEquipTooltip != null)
        {
            Destroy(currentActiveEquipTooltip);
