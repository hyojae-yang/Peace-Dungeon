...
public class WolfBehavior : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        {
            Debug.LogError("WolfBehavior: 필수 컴포넌트(Monster, MonsterPatrol, MonsterCombat, Animator) 중 일부를 찾을 수 없습니다.");
            enabled = false;
            return;
        }

        // AudioSource는 선택 사항으로 간주하고, 없으면 경고만 표시
        if (audioSource == null)
        {
            Debug.LogWarning("WolfBehavior: AudioSource 컴포넌트를 찾을 수 없습니다. 사운드 재생이 비활성화됩니다.");
        }
    ...
    private void HandleChase(float distanceToPlayer)
    {
        ...
        {
            // 리더가 아닐 경우, 단순히 Chase를 끝내고 Patrol로 돌아갑니다.
            if (!isLeader)
            {
                ExitPackInternal(); // leader 참조는 이미 DissolvePack에서 null이므로 안전
            }
            else
            {
                // 리더는 무리를 완전히 해산시킵니다.
    ...
    private void HandleFlocking(float distanceToPlayer)
    {
        ...
        else // leader가 null이 되어 무리가 해제되었는데도 Flocking 상태인 경우 (예외 처리)
        {
            monster.ChangeState(MonsterBase.MonsterState.Chase);
        }
    ...
    public void JoinPack(WolfBehavior newLeader)
    {
        ...
        // 리더 주변, 플레이어 반대 방향의 고정 위치를 합류 목표로 설정합니다.
    ...
    /// 모든 추종자들에게 해산 명령을 내립니다.
    /// </summary>
    private void ExitPack()
    {
        // 1. 리더 자신을 제외한 모든 추종자들에게 해산 명령을 내립니다.
        foreach (var follower in followers)
        {
            if (follower != null)
            {
                // 추종자에게 DissolvePack 명령을 내려 자립하도록 합니다.
                follower.DissolvePack();
            }
        }
        ...
        // 3. 리더 자신은 내부 해제 로직을 통해 Patrol로 전환
        ExitPackInternal();
    }

    /// <summary>
    /// 추종자 늑대가 리더로부터 해제될 때 호출됩니다.
    /// 리더-추종자 관계를 해제하고, 플레이어와의 거리에 따라 **Chase 또는 Patrol로 전환하여 자립**하도록 합니다.
    /// </summary>
    public void DissolvePack()
    {
        // 1. 리더-추종자 관계 해제 및 플래그 초기화
        leader = null;
        isJoinedPack = false;
        initialFlockTarget = Vector3.zero;
        ...
        if (playerTransform != null)
        {
            ...
            // 감지 범위 내에 플레이어가 남아있다면
            if (distanceToPlayer <= monster.detectionRange)
            {
                // 플레이어가 아직 근처에 있다면 Chase 상태로 복귀 (자립 공격)
                monster.ChangeState(MonsterBase.MonsterState.Chase);
                // Chase 상태에서는 MonsterPatrol.StopPatrol()을 호출하여 이동을 직접 제어해야 합니다.
                if (monsterPatrol != null) monsterPatrol.StopPatrol();
            }
            else
            {
                // 플레이어를 놓쳤다면 Patrol 상태로 전환 (완전 해산)
                monster.ChangeState(MonsterBase.MonsterState.Patrol);
            }
        }
        else
        {
            // 플레이어 참조가 없다면 Patrol로
            monster.ChangeState(MonsterBase.MonsterState.Patrol);
        }
    }
    ...
    /// <summary>
    /// 무리 이탈의 실제 로직을 수행합니다. (리더 본인 및 Chase 상태 이탈 시 공용 사용)
    /// </summary>
    ...
    /// <summary>
    /// 특정 위치(Vector3)를 향해 이동하는 공통 로직. (OCP 준수, 이제 회전 로직이 제거되어 순수 이동만 수행)
    /// </summary>
    /// <param name="targetPosition">목표 Vector3 위치</param>
    /// <param name="speed">이동 속도</param>
    /// <param name="stoppingDistance">목표에 도달했다고 판단할 최소 거리</param>
