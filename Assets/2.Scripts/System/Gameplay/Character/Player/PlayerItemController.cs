...
public class PlayerItemController : MonoBehaviour, ISavable
{
    ...
    private const KeyCode STARTING_KEY_CODE = KeyCode.Alpha5; // 퀵슬롯 시작 키 (5번 키: 5, 6, 7, 8)
...
    // === 데이터 ===
    [Header("아이템 퀵슬롯 할당")]
    [Tooltip("5~8 키에 할당할 소모품 아이템 데이터 (ConsumableItemSO)를 할당합니다. [데이터 접근 제한 강화]")]
    [SerializeField] // 인스펙터에 노출하여 할당은 가능하게 함
    // [수정] public에서 private으로 변경하여 외부에서 itemSlots 배열을 직접 덮어쓰는 것을 방지
    private ConsumableItemSO[] itemSlots = new ConsumableItemSO[SLOT_COUNT]; // 퀵슬롯에 등록된 아이템 데이터 배열
...
        playerCharacter = PlayerCharacter.Instance;
        ...
        {
            Debug.LogError("[PITC] 핵심 종속성(PlayerCharacter, InventoryManager, ItemDatabaseManager) 중 하나 이상을 찾을 수 없습니다.");
            enabled = false;
            return; // 종속성 실패 시 중단
        }
...
    public void UseQuickSlotItem(int slotIndex)
    {
        ...
        {
            Debug.LogError($"[PITC][QuickSlot Error] 잘못된 퀵슬롯 인덱스입니다: {slotIndex}");
            return;
        }
...
            OnSlotItemChanged?.Invoke(slotIndex, itemToRegister);
        ...
        {
            Debug.LogError("[PITC] 잘못된 슬롯 인덱스입니다: " + slotIndex);
        }
    }
...
                            itemSlots[i] = itemToAssign;
                        ...
                        {
                            // 아이템 데이터를 찾을 수 없거나 타입이 일치하지 않는 경우
                            itemSlots[i] = null;
                        }
                    }
                    else
                    ...
                    // [수정] 로드 완료 후 UI에 최종 상태를 알려줍니다. 
                    // QuickSlotItemPanel이 Start()에서 구독 후 RefreshAllSlotQuantities()를 호출하지만, 
                    // 만약을 대비하여 로드된 데이터를 기반으로 UI 동기화 이벤트를 명시적으로 발생시킵니다.
