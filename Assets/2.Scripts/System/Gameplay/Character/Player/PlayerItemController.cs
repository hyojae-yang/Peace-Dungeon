...
public class PlayerItemController : MonoBehaviour, ISavable
{
    ...
    {
        // Debug.Log($"[QuickSlot Use] UseQuickSlotItem 호출됨 (인덱스: {slotIndex})");
        ...
        // 참고: 사용 성공 여부 및 수량 감소 로직은 InventoryManager에 있으므로, 
        // 이 다음에는 InventoryManager의 로그를 확인해야 합니다.
    }
    ...
    // === ISavable 인터페이스 구현 ===
    // ... SaveData 및 LoadData 메서드는 디버그 로그가 필요하지 않으므로 그대로 유지합니다.
