...
public class PlayerAttack : MonoBehaviour
{
    ...
    // 플레이어의 AudioSource 컴포넌트입니다.
    private AudioSource playerAudioSource; // AudioSource 참조
    ...
    // === 오디오 클립 설정 ===
    [Header("오디오 클립")] // 오디오 클립 할당을 위한 헤더
    [Tooltip("근접 공격 시 재생할 효과음 클립입니다. (예: 칼 휘두르는 소리)")]
    public AudioClip meleeAttackClip; // 근접 공격 시전 소리
    [Tooltip("원거리 공격 시 재생할 효과음 클립입니다. (예: 활 시위 당기는 소리, 마법 시전 소리)")]
    public AudioClip rangedAttackClip; // 원거리 공격 시전 소리

    // ⭐ 추가: 일반 타격 성공음 클립
    [Tooltip("공격 성공 시 일반 데미지가 들어갔을 때 재생할 효과음 클립입니다. (치명타 아닐 때)")]
    public AudioClip hitImpactClip;

    [Tooltip("공격 성공 시 치명타가 터졌을 때 재생할 효과음 클립입니다.")]
    public AudioClip criticalHitClip; // 치명타 소리
    ...
    private void Start()
    {
        ...
        {
            Debug.LogError("PlayerAttack 스크립트가 붙은 오브젝트에 AudioSource 컴포넌트가 없습니다!");
        }
    ...
    private IEnumerator AttackWithDelay()
    {
        ...
        // 1. 공격 시전 사운드 재생
        PerformAttackSound(false, true); // 시전 사운드는 치명타 여부와 관계없이 재생합니다.
    ...
    /// <summary>
    /// 무기 타입과 치명타 여부에 따라 공격 사운드를 재생합니다.
    /// </summary>
    /// <param name="isCritical">현재 공격이 치명타인지 여부</param>
    /// <param name="isStartSound">공격 시작(시전) 사운드인지, 적중 사운드인지 여부 (true: 시전, false: 적중/치명타)</param>
    private void PerformAttackSound(bool isCritical, bool isStartSound) // 사운드 재생 헬퍼 메서드
    {
        if (playerAudioSource == null || equippedWeapon == null) return;

        AudioClip clipToPlay = null;

        if (isStartSound) // 공격 시전 사운드 (딜레이 전)
        {
        ...
        else // 적중/치명타 사운드 (딜레이 후 - 근접 공격 타격음)
        {
            // 치명타 성공 시 치명타 사운드를 재생합니다.
            if (isCritical)
            {
                clipToPlay = criticalHitClip;
            }
            // ⭐ 수정: 치명타가 아닐 경우에만 일반 타격 성공음을 재생합니다.
            else
            {
    ...
    /// <summary>
    /// 근접 공격 로직을 실행합니다. (콜라이더의 중심점이 아닌, 영역 겹침 기반으로 각도 판정)
    /// </summary>
    ...
    private void PerformMeleeAttack(float damage, bool isCritical)
    {
        ...
        // 공격 범위 내의 몬스터를 찾습니다. (콜라이더 영역이 겹치면 모두 감지)
        ...
        {
            // ⭐ 핵심 수정 포인트: 몬스터 콜라이더의 '가장 가까운 지점'을 찾습니다.
            // 플레이어 시점에서 봤을 때, 몬스터 콜라이더의 가장자리가 부채꼴 안에 들어오는지 확인하기 위함입니다.
            Vector3 closestPoint = monsterCollider.ClosestPoint(transform.position);

            // 플레이어 중심에서 몬스터의 '가장 가까운 지점'으로 향하는 방향 벡터
            Vector3 direction3D = closestPoint - transform.position;
            ...
            // 3. 평탄화된 전방 벡터와 '가장 가까운 지점' 방향 벡터 사이의 각도를 계산합니다.
            float angle = Vector3.Angle(forwardFlat.normalized, directionFlat.normalized);

            // **참고:** OverlapSphere에서 이미 거리가 체크되었으므로,
            // 이곳에서는 각도만 체크하고 거리는 생략할 수 있습니다. (OverlapSphere가 이미 겹침을 보장)

            /* if (directionFlat.magnitude > currentAttackRange)
            {
                 continue; // 이 코드는 OverlapSphere 덕분에 불필요함.
            }
            */

            // 몬스터의 콜라이더 '가장 가까운 지점'이 공격 각도 범위 안에 있는지 확인합니다.
            if (angle < currentAttackAngle * 0.5f)
            {
                ...
                if (damageableTarget != null)
                {
                    ...
                    PerformAttackSound(isCritical, false); // 타격 성공 사운드

                    // 넉백 로직 (방향은 여전히 중심 기준 또는 ClosestPoint 기준으로 유지할 수 있습니다.)
                    if (equippedWeapon.knockbackForce > 0)
                    {
                        Rigidbody monsterRb = monsterCollider.GetComponent<Rigidbody>();
                        if (monsterRb != null)
                        {
                            // 넉백 방향은 몬스터의 중심(Collider.transform.position)을 이용해 밀어내는 것이 일반적입니다.
    ...
    private void PerformRangedAttack(float damage, bool isCritical)
    {
        ...
        if (equippedWeapon.projectilePrefab != null)
        {
            ...
            else
            {
                ...
                {
                    // 수정된 Projectile.SetProjectileData 시그니처에 맞게 isCritical을 추가 전달
                    projectileComponent.SetProjectileData(equippedWeapon, monsterLayer, damage, isCritical);

                    // 원거리 공격의 적중 사운드는 발사체 스크립트(Projectile.cs)에서 처리합니다.
                }
                else
                {
