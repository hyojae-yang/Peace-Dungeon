...
public class DungeonMap : MonoBehaviour
{
    ...
    /// <summary>
    /// Denial Tile을 제외한, 플레이어(SmallMap)가 현재 점유하고 있는 타일의 총 개수를 반환합니다.
    /// (4칸짜리 맵을 배치했다면 4로 카운트됩니다.)
    /// </summary>
    /// <returns>순수하게 플레이어가 배치한 타일의 갯수 (int)</returns>
    public int GetPlayerOccupiedTileCount()
    {
        int count = 0;
        // occupiedTiles 딕셔너리의 '값' (Transform)을 순회합니다. 각 값은 하나의 그리드 타일(Key)에 대응합니다.
        foreach (var tileTransform in occupiedTiles.Values)
        {
            // Denial Tile (빌리지 맵)을 제외하고, SmallMap에 의해 점유된 타일만 카운트합니다.
            // DUMMY_DENIAL_MAP은 Awake에서 this.transform으로 초기화되었거나, 
            // 아예 다른 별도의 더미 오브젝트일 수 있으므로, 정확히 DUMMY_DENIAL_MAP과 비교합니다.
