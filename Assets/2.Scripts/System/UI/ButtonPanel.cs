...
public class ButtonPanel : MonoBehaviour
{
    ...
    /// <summary>
    /// '퀵슬롯 등록' 버튼 클릭 시 호출됩니다. (코루틴 시작)
    /// 첫 클릭 누락 문제를 해결하기 위해 실제 로직은 코루틴으로 지연합니다.
    /// </summary>
    public void OnRegisterQuickSlotButtonClicked()
    {
        // 첫 클릭 누락 문제를 해결하기 위해 로직 실행을 한 프레임 지연시킵니다.
        StartCoroutine(Co_RegisterQuickSlotButtonClicked());
    }

    /// <summary>
    /// 퀵슬롯 선택 패널 활성화 로직을 다음 프레임에 수행합니다. (첫 클릭 이슈 해결)
    /// </summary>
    private IEnumerator Co_RegisterQuickSlotButtonClicked()
    {
        // **핵심 수정:** 다음 프레임까지 대기하여 UI 시스템의 레이캐스트 업데이트를 허용합니다.
        yield return null;
