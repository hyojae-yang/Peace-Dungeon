...
public class RewardTextManager : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        // PlayerLevelUp이 static 이벤트이므로, null 체크 없이 구독합니다. (PlayerLevelUp이 항상 존재한다고 가정)
        // PlayerLevelUp 클래스 자체가 로드되지 않았다면 Null 참조가 발생할 수 있으므로 주의 필요
    ...
    public void ShowReward(long amount, bool isGold)
    {
        ...
        // [수정된 부분] 캔버스 활성화 상태 체크: 캔버스가 비활성화되어 있다면 UI 생성을 시도하지 않습니다.
        // 이는 UI 스크립트가 비활성화된 부모 아래에서 생성될 때 발생하는 Null 참조 예외를 방지합니다.
        if (!targetCanvas.gameObject.activeInHierarchy)
        {
            // Debug.Log("[RewardTextManager] 타겟 캔버스 비활성화 상태. 팝업 생성 스킵."); 
            return;
        }
