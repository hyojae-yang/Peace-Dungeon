...
/// [BGM 피치 제어 추가] 4. 던전 탐험 중 현재 위험도 레벨/게이지에 따라 BGM 피치를 계산하고 SoundManager에 전달.
/// </summary>
...
    // =======================================================
    // [팩트 2: BGM 피치 제어 상수 추가]
    // 논의 1B (점진적 긴장감 강화) / 논의 2B (게이지 직접 연동) 지원
    // =======================================================

    /// <summary>
    /// BGM 피치 변화 패턴이 반복되는 위험도 레벨 주기입니다. (예: 5레벨마다 패턴 갱신)
    /// </summary>
    public const int PITCH_CHANGE_CYCLE_LEVEL = 5;

    /// <summary>
    /// 레벨 0~4 구간의 BGM 기본 최소 피치입니다.
    /// </summary>
    private const float BASE_MIN_PITCH = 0.9f;

    /// <summary>
    /// 레벨 0~4 구간의 BGM 기본 최대 피치입니다.
    /// </summary>
    private const float BASE_MAX_PITCH = 1.1f;

    /// <summary>
    /// 5레벨 주기(사이클)마다 피치 범위를 확장할 증분 값입니다. (점진적 강화)
    /// 즉, 5레벨마다 Min은 0.02f 감소, Max는 0.02f 증가합니다.
    /// </summary>
    private const float PITCH_RANGE_EXPANSION_PER_CYCLE = 0.02f;
...
    /// <summary>
    /// 현재 위험도 레벨입니다. (계산된 값, 난이도 감속 인자 및 피치 계산에 사용됨)
    /// </summary>
    private int currentRiskLevel = 0;

    /// <summary>
    /// 현재 던전 탐험 중인지 나타내는 플래그입니다. (게이지 상승 모드 및 피치 변조 활성화 여부)
    /// </summary>
    private bool isExploring = false;

    /// <summary>
    /// 현재 레벨에서 누적된 게이지 진행도 값입니다. (0.0f ~ 1.0f)
    /// 시간에 따라 증가하는 실질적인 게이지 값이며, BGM 피치 계산의 비율로 사용됩니다.
    /// </summary>
...
    /// <summary>
    /// [핵심 기능] 매 프레임마다 게이지를 시간에 따라 업데이트하고 UI 및 BGM에 반영합니다.
    /// </summary>
    private void Update()
    {
...
            // [신규] 플레이어가 점유한 타일 갯수를 DungeonMap에서 가져옵니다.
            int occupiedTileCount = 0;
            // DungeonMap이 싱글톤이라고 가정하고 인스턴스를 가져옵니다.
...
            // [BGM 피치 제어 로직 추가]
            float targetPitch = CalculateBGMPitch(currentRiskLevel, currentLevelProgress);

            if (SoundManager.Instance != null)
            {
                // 계산된 목표 피치를 SoundManager에 전달합니다.
                SoundManager.Instance.SetTargetBGMPitch(targetPitch);
            }
        }
        else // 던전 탐험 중이 아닐 때 (isExploring == false)
        {
            // 던전 퇴장 시 BGM 피치를 정상값(1.0f)으로 리셋하도록 SoundManager에 명령
            if (SoundManager.Instance != null)
            {
                SoundManager.Instance.SetTargetBGMPitch(1.0f);
            }
        }
...
    // =======================================================
    // [BGM 피치 계산 로직 추가] (핵심 추가)
    // =======================================================

    /// <summary>
    /// 현재 위험도 레벨과 게이지 진행도에 기반하여 BGM의 목표 피치 값을 계산합니다.
    /// (논의 1B: 점진적 강화) (논의 2B: 게이지 직접 연동)
    /// </summary>
    /// <param name="riskLevel">현재 위험도 레벨</param>
    /// <param name="gaugeProgress">현재 레벨 내 게이지 진행도 (0.0f ~ 1.0f)</param>
    /// <returns>적용할 BGM 피치 값</returns>
    private float CalculateBGMPitch(int riskLevel, float gaugeProgress)
    {
        // 1. 현재 레벨이 속한 주기(사이클) 횟수를 계산합니다.
        // 0~4레벨: cycleCount = 0 (기본)
        // 5~9레벨: cycleCount = 1 (1회 확장)
        // 10~14레벨: cycleCount = 2 (2회 확장)
        int cycleCount = riskLevel / PITCH_CHANGE_CYCLE_LEVEL;

        // 2. 주기(cycleCount)에 따라 피치 범위(Min/Max)를 점진적으로 확장합니다. (논의 1B)
        float pitchExpansion = cycleCount * PITCH_RANGE_EXPANSION_PER_CYCLE;

        // Min 피치는 감소, Max 피치는 증가하여 범위가 확장됩니다.
        float currentMinPitch = BASE_MIN_PITCH - pitchExpansion;
        float currentMaxPitch = BASE_MAX_PITCH + pitchExpansion;

        // 최소 피치 제한 (BGM이 너무 느려지는 것을 방지)
        currentMinPitch = Mathf.Max(0.5f, currentMinPitch);

        // 3. 현재 레벨 내 게이지 진행도(0.0f ~ 1.0f)에 따라 Min Pitch에서 Max Pitch까지 선형 보간합니다. (논의 2B)
        // 게이지가 찰수록(0.0f -> 1.0f) 피치가 빨라지고(Min -> Max), 레벨업 시 게이지가 리셋되면 피치도 다시 느려집니다.
        float targetPitch = Mathf.Lerp(
      currentMinPitch,
      currentMaxPitch,
      gaugeProgress // currentLevelProgress (0.0f ~ 1.0f) 값을 비율로 직접 사용합니다.
        );
...
    public void StopExploration()
    {
        isExploring = false;
        // 던전 퇴장 시 BGM 피치를 즉시 1.0f으로 되돌리도록 SoundManager에 명령합니다.
        if (SoundManager.Instance != null)
        {
            SoundManager.Instance.SetTargetBGMPitch(1.0f);
        }
